...
bool LandAndWavesApp::CubbyFlowInitialize()
{
	mFps = 60;
	mFrame = Frame(0, 1.0 / mFps);
	mNumberOfFrames = 100;
	mResX = 50;
...
//	RunExample1();
	...
	mSolver = LevelSetLiquidSolver3::Builder()
		.WithResolution({ mResX, 2 * mResX, mResX })
		.WithDomainSizeX(1.0)
		.MakeShared();
...
XMFLOAT3 Vector3DToXMFLOAT3(Vector3D v)
{
	return XMFLOAT3((float)v.x, (float)v.y, (float)v.z);
}

void LandAndWavesApp::UpdateMaterial(const GameTimer& gt)
{
	if ((int)gt.TotalTime() % 33 == 0)
	{
		mSolver->Update(mFrame++);
		TriangulateAndSave();

		auto currWaterVB = mCurrFrameResource->WaterVB.get();
		auto currWaterIB = mCurrFrameResource->WaterVB.get();

		for (size_t i = 0; i < mMesh.NumberOfPoints(); ++i)
		{
			Vertex v;

			v.Pos = Vector3DToXMFLOAT3(mMesh.Point(i));
...
void LandAndWavesApp::TriangulateAndSave()
{
...
void LandAndWavesApp::ConvertVertexData()
{
	...
	geo->DrawArgs["water"] = liquidSubmesh;
...
void LandAndWavesApp::BuildWaterGeometryBuffers()
{
	mSolver->Update(mFrame++);
	TriangulateAndSave();
	ConvertVertexData();
...
void LandAndWavesApp::BuildFrameResources()
{
    for(int i = 0; i < gNumFrameResources; ++i)
    {
        mFrameResources.push_back(std::make_unique<FrameResource>(md3dDevice.Get(),
            1, (UINT)mAllRitems.size(), mMesh.NumberOfPoints()));
    }
}
...
void LandAndWavesApp::BuildRenderItems()
{
	...
	liquidRitem->Geo = mGeometries["waterGeo"].get();
	liquidRitem->PrimitiveType = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
	liquidRitem->IndexCount = liquidRitem->Geo->DrawArgs["grid"].IndexCount;
	liquidRitem->StartIndexLocation = liquidRitem->Geo->DrawArgs["grid"].StartIndexLocation;
	liquidRitem->BaseVertexLocation = liquidRitem->Geo->DrawArgs["grid"].BaseVertexLocation;

	mWaterRitem = liquidRitem.get();
